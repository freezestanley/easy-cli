  ...
  <div class="asset-wrap">
    <div class="info">
      ...
      <div class="item">
        ...
        <p v-if="assetType === PAY_ASSETS">
          <label>应付金额</label>
          <span>{{ assetInfo.realAmt | formatWan }}万</span>
        </p>
        <p v-else>
          <label>应收金额</label>
          <span>{{ assetInfo.realAmt | formatWan }}万</span>
        ...
        <p v-if="assetType === PAY_ASSETS">
          <label>供应商名称</label>
          <span>{{ assetInfo.receiveCorpName }}</span>
        </p>
        <p v-else>
          <label>客户名称</label>
          <span>{{ assetInfo.payCorpName }}</span>
        </p>
      </div>
      <div class="item">
        <p v-if="assetType === PAY_ASSETS">
          <label>应付到期日</label>
          <span>{{ assetInfo.assetExpireDate }}</span>
        </p>
        <p v-else>
          <label>应收到期日</label>
          <span>{{ assetInfo.assetExpireDate }}</span>
        ...
        <p>
          <label>合同编号</label>
          <span>{{ assetInfo.contractNo }}</span>
        </p>
        <p>
          <label>合同金额</label>
          <span>{{ assetInfo.contractAmt | formatWan }} 万</span>
        </p>
      </div>
      <div class="item">
        <p>
          <label>合同日期</label>
          <span>{{ assetInfo.contractDate }}</span>
        </p>
        <p>
          <label>备注</label>
          <span>{{ assetInfo.remark }}</span>
        </p>
        <p />
      </div>
    </div>
    ...
    <div class="info">
      ...
      <div><label>总金额</label> <strong> {{ assetInfo.invoiceTotalAmount | formatWan }} 万</strong></div>
...
export default {
  ...
  computed: {
    contractList() {
      return this.assetInfo.attaches.filter(i => i.assetAttachType === 'CONTRACT')
    },
    invoiceList() {
      return this.assetInfo.attaches.filter(i => i.assetAttachType === 'INVOICE')
    },
    settlementList() {
      return this.assetInfo.attaches.filter(i => i.assetAttachType === 'SETTLEMENT')
    },
    otherList() {
      return this.assetInfo.attaches.filter(i => i.assetAttachType === 'OTHER')
    },
  },
  methods: {
