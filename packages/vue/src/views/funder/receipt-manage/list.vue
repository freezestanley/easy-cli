  ...
  <div class="page">
    <div class="filter-form">
      <el-form ref="ruleForm" :inline="true" :model="ruleForm" label-position="left">
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="收款日期" prop="range">
              <el-date-picker
    ...
    <div class="table-box">
      <basic-table v-loading="loading" stripe :data="list" :col-configs="colConfigs">
        ...
        <el-table-column slot="status" label="收款状态" fixed="right">
          <template slot-scope="scope">
        ...
        <el-table-column slot="opt" label="操作" width="240" fixed="right">
          <template slot-scope="scope">
            <div>
              <el-button v-permission="'receipt-manage-list__view'" type="text" @click="viewReceiptDetail(scope.row)">查看收款</el-button>
              <el-button v-permission="'receipt-manage-list__contract'" type="text" @click="viewContract(scope.row)">查看合约</el-button>
            </div>
          </template>
        </el-table-column>
        <div slot="append">
...
export default {
  ...
  data() {
    return {
      ...
      colConfigs: [
        ...
        { label: `收款${this.TOKEN__NAME}总数（个）`, prop: 'tokenCount', width: '150px' },
        { label: `已收${this.TOKEN__NAME}数（个）`, prop: 'paidTokenCount', width: '150px' },
        { label: `待收${this.TOKEN__NAME}数（个）`, prop: 'unpaidTokenCount', width: '150px' },
        { label: '收款日期', prop: 'dueDate', width: '150px', formatter: (row) => { return this.formatterTime(row.dueDate, 'YYYY-MM-DD') } },
  ...
  methods: {
    ...
    viewReceiptDetail(row) {
      this.$router.push({
        path: '/business/receipt/detail',
        query: {
          repayPlanNo: row.repayPlanNo,
          collectionCorpNo: row.collectionCorpNo,
          tokenNo: row.tokenNo,
        }
      })
    },
