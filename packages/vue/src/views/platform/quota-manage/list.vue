<template>
  <div class="page">
    ...
    <div class="table-box">
      <basic-table v-loading="loading" stripe :data="list" :col-configs="colConfigs">
        ...
        <el-table-column slot="opt" label="操作" width="240" fixed="right">
          <template slot-scope="scope">
            <div v-if="scope.row.status === 'AVALIABLE'">
              <el-button v-permission="['PLATFORM_6']" type="text" @click="changeStatus(scope.row)">冻结</el-button>
              <el-button v-permission="['PLATFORM_6']" type="text" @click="adjust(scope.row)">调额</el-button>
              <el-button v-permission="['PLATFORM_6']" type="text" @click="extensionTime(scope.row)">延长有效期</el-button>
              <el-button type="text" @click="viewCreditDetail(scope.row.quotaNo)">授信明细</el-button>
            </div>
            <div v-else-if="scope.row.status === 'FREEZED'">
              <el-button v-permission="['PLATFORM_6']" type="text" @click="changeStatus(scope.row)">解冻</el-button>
              <el-button type="text" @click="viewCreditDetail(scope.row.quotaNo)">授信明细</el-button>
            </div>
            <div v-else>
              <el-button type="text" @click="viewCreditDetail(scope.row.quotaNo)">授信明细</el-button>
    ...
    <!-- 延长有效期 -->
    <extension-time-dialog ref="extensionTimeDialog" v-permission="['PLATFORM_6']" />

    <!-- 调额 -->
    <adjust-dialog ref="adjustDialog" v-permission="['PLATFORM_6']" />

    <!-- 解冻/冻结 -->
    <status-dialog ref="statusDialog" v-permission="['PLATFORM_6']" />
  ...
  data () {
    return {
      ...
      colConfigs: [
        { slot: 'txHash' },
        { slot: 'corpName' },
        { label: '授信时间', prop: 'issueDate', width: '150px', formatter: (row) => { return this.formatterTime(row.issueDate) } },
  ...
  methods: {
    ...
    extensionTime(row) {
      this.$refs.extensionTimeDialog.open({
        initiator: 'platform',
        quotaNo: row.quotaNo,
        coreCorpNo: row.corpNo,
        fundCorpNo: row.issueCorpNo,
    ...
    changeStatus(row) {
      this.$refs.statusDialog.open({
        ...
        fundCorpNo: row.issueCorpNo,
        corpName: row.corpName,
        showNo: row.status === 'FREEZED' ? row.balAmt : row.balAmt, // 2019 7 12
