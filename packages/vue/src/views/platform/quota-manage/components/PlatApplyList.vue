      ...
      <el-form ref="ruleForm" :model="ruleForm" :inline="true" label-position="left" label-width="96px">
        <el-row :gutter="20">
          ...
          <el-col :span="8" class="tar">
            <el-form-item label="申请内容" prop="quotaOperateTransType">
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ...
        bizStatus: '',
        transNo: '',
        triggerCorpType: 'PLATFORM',
      },
      loading: false,
      list: [],
      colConfigs: [
        { label: '编号', prop: 'transNo', width: '150px', 'show-overflow-tooltip': true },
        { label: '申请时间', prop: 'gmtCreated', width: '150px', formatter: (row) => { return this.formatterTime(row.gmtCreated, 'YYYY-MM-DD') } },
        { slot: 'toCorpName' },
        ...
        { label: '申请内容', prop: 'transType', width: '200px', 'show-overflow-tooltip': true, formatter: this.formatterTransType },
        { label: '备注', prop: 'reason', minWidth: '200px', 'show-overflow-tooltip': true },
        { slot: 'attachments' },
        { slot: 'bizStatusName' },
        { slot: 'opt' },
      ],
  ...
  computed: {
    ...
    bizStatusArr: function() {
      return this.currentDict.PLATFORM_APPLY_ADJUST_CREDIT || []
    }
  },
  watch: {
  ...
  methods: {
    ...
    //     flowTypes: ['PLATFORM_APPLY_ADJUST_CREDIT'],
    //   }).then(res => {
