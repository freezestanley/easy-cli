  ...
  <div>
    <div class="filter-form">
      <el-form ref="ruleForm" :model="ruleForm" :inline="true" label-position="left">
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="核心企业名称" prop="toCorpName" label-width="96px">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="编号" prop="transNo" label-width="96px">
              <el-input
    ...
    <div class="table-box">
      <basic-table v-loading="loading" stripe :data="list" :col-configs="colConfigs">
        <el-table-column slot="toCorpName" label="核心企业名称" width="150px" show-overflow-tooltip>
          <template slot-scope="scope">
            <corp-link :corp-no="scope.row.toCorpNo">{{ scope.row.toCorpName }}</corp-link>
          </template>
        </el-table-column>
        <el-table-column slot="attachments" label="附件" width="180px">
          <template slot-scope="scope">
            <div v-for="(item, index) in scope.row.attachments" :key="index" class="ellipsis cursor">
              <a :href="item" target="_blank" class="theme hover">{{ getUrlFileName(item) }}</a>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column slot="opt" label="操作" width="140px" fixed="right">
          <template slot-scope="scope">
            <div>
              ...
              <el-button v-if="scope.row.button === CHECK_FLOW_STATUS.OPERATE && checkPermission('quota-apply__check')" type="text" @click="showCheck(scope.row)">审核</el-button>
...
import { AQUOTA_OPERATE_LIST, AQUOTA_OPERATE_MAP, } from '@/utils/constant/quota'
...
export default {
  ...
  data() {
    return {
      ...
      colConfigs: [
        ...
        { label: '申请时间', prop: 'gmtCreated', width: '150px', },
        { slot: 'toCorpName' },
        ...
        { label: '申请内容', prop: 'transType', width: '200px', formatter: (row) => { return AQUOTA_OPERATE_MAP[row.transType] + '至' + row.transParam.changeTargetQuota + '个' } },
  ...
  methods: {
    ...
    //     flowTypes: ['CORE_APPLY_ADJUST_CREDIT'],
    //   }).then(res => {
