<template>
  <div class="upload">
    ...
    >
      ...
      <div slot="tip" class="el-upload__tip">只能上传 jpg/png文件，且不超过 2Mb</div>
    </el-upload>
...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      const types = ['jpg', 'png']
      const isJPG = types.includes(this.getFileType(file.name))
      const isLt5M = file.size / 1024 / 1024 < 2
      if (!isJPG) {
        this.$message.error('上传只能是 png/jpg 格式!')
      }
      if (!isLt5M) {
        this.$alert(`<div>上传文件不能超过2M！</div>`, '提示', {
